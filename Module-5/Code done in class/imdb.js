...
function click_button(button){
	...
	if(button === 'right' && i <= data.results.length-2){
		i++;
	}
	else{
		i--;
...
function load(index) {
	console.log(index);
	document.getElementById("movie_poster").innerHTML =  '<img class="img-responsive" src="https://image.tmdb.org/t/p/w500/' + data.results[index].poster_path +'" alt="Movie Poster">';
	document.getElementById("rating").innerHTML = data.results[index].vote_average;
	document.getElementById("movie_title").innerHTML = data.results[index].original_title;
	document.getElementById("release_date").innerHTML = data.results[index].release_date;
	document.getElementById("movie_overview").innerHTML = data.results[index].overview;
};
